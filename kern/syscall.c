...
sys_exofork(void)
{
	...
	e->env_tf.tf_regs.reg_eax = 0; // Return value of new env is 0
	return e->env_id;
}
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	if (((uintptr_t)va >= UTOP) || (va != ROUNDDOWN(va, PGSIZE)))
		return -E_INVAL;
	if ((perm & (PTE_U | PTE_P)) != (PTE_U | PTE_P)) // must set
		return -E_INVAL;
	if (perm & ~PTE_SYSCALL) // may or may not set
		return -E_INVAL;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if ((uintptr_t)srcva >= UTOP || (uintptr_t)dstva >= UTOP || 
		...
		return -E_INVAL;
	pte_t *src_pte;
	struct PageInfo *pp = page_lookup(src_env->env_pgdir, srcva, &src_pte); // pp map at srcva
	if (!pp)
		return -E_INVAL;
	if ((perm & (PTE_U | PTE_P)) != (PTE_U | PTE_P)) // must set
		return -E_INVAL;
	...
	if ((perm & PTE_W) && !(*src_pte & PTE_W))
		return -E_INVAL;
	int ret = page_insert(dst_env->env_pgdir, pp, dstva, perm);
	return ret;
}
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
    ...
    e->env_ipc_recving = 0;		// block future sends 
    e->env_ipc_from = curenv->env_id;
    e->env_ipc_value = value;
    e->env_status = ENV_RUNNABLE;
    e->env_tf.tf_regs.reg_eax = 0;		// Receiver "returns" 0!!!
    return 0;
}
