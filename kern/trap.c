...
trap_init(void)
{
	...
	// SETGATE(idt[T_BRKPT], 1, GD_KT, trap_handler3, 0);
	...
	for (i = 0; i <= 19; i++) {
		if (i == 9 || i == 15) {
			...
			SETGATE(idt[i], 1, GD_KT, trap_entry_arr[i], 3);
		}
		else {
			SETGATE(idt[i], 1, GD_KT, trap_entry_arr[i], 0);
		}
	}
	SETGATE(idt[T_SYSCALL], 0, GD_KT, trap_handler_syscall, 3);
